namespace BoneBoard.Modules.Blockers
{
    internal class CustomEmojisAndStickers(BoneBot bot) : ModuleBase(bot)
    {
        ...
        private static bool IsProhibitedIn(DiscordUser? user, DiscordChannel? channel)
        {
            if (user is null || channel is null)
                return false;

            return Config.values.channelsWhereUsersAreProhibitedFromCustomEmojis.TryGetValue(channel.Id.ToString(), out var emojiUserIds) &&
                   !emojiUserIds.Contains(user.Id);
        }
    }
}