...
internal class ImageRoyale : ModuleBase
{
    ...
    protected override async Task FetchGuildResources()
    {
        ...
        try
        {
            if (ulong.TryParse(Config.values.imageRoyaleVoteEmoji, out ulong emojiId))
                voteEmoji = DiscordEmoji.FromGuildEmote(bot.client, emojiId);
            else if (DiscordEmoji.TryFromUnicode(Config.values.imageRoyaleVoteEmoji, out DiscordEmoji unicodeEmoji))
