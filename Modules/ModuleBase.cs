...
namespace BoneBoard.Modules
{
    public abstract partial class ModuleBase
    {
        ...
        public ModuleBase(BoneBot bot) : this()
        {
            ...
            {
                AllModules.Add(this);
                bot.clientBuilder.ConfigureServices(x => x.AddSingleton(GetType(), this));
            }
            ...
            bot.ConfigureEvents(x => x
                ...
                .HandleMessageDeleted(AllEventsHandler)
                .HandleMessageReactionAdded(AllEventsHandler)
                .HandleMessageReactionRemoved(AllEventsHandler)
        ...
        {
            var hasBlocker = InstanceMethod(GlobalStopEventPropagation).DeclaringType == GetType();
            var handlersThatNeedRegistering = new List<(bool needsRegister, Action<EventHandlingBuilder> builder, string eventName)>
            {
                ...
                (InstanceMethod(ReactionRemoved).DeclaringType == GetType(),
                    ...
                    "ReactionRemoved"),
                (InstanceMethod(GuildMemberAdded).DeclaringType == GetType(),
                    builder => builder.HandleGuildMemberAdded(GuildMemberAddedEvent),
                    "GuildMemberAdded"),
            ...
            Logger.Put(
                $"Configured event handlers for {GetType().Name}, adding {handlersThatNeedRegistering.Count(x => x.needsRegister)} events{(hasBlocker ? " in addition to its blocker" : "")}.");
        }
        ...
        private static async Task ConfigChanged()
        {
        ...
        private static readonly CircularBuffer<DiscordEventArgs> DontPropagate = new(4096);
