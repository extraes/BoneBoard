...
namespace BoneBoard
{
    internal static class Logger
    {
        ...
        static Logger()
        {
            var maxFiles = Config.values.maxLogFiles;
            if (!Directory.Exists(Config.values.logPath)) Directory.CreateDirectory(Config.values.logPath);
            var filePath = Path.Combine(Config.values.logPath, DateTime.Now.ToString(FILE_DATE_FORMAT) + ".log");
            var latestPath = Path.Combine(Config.values.logPath, "latest.log");
            ...
            var files = dir.GetFiles().OrderByDescending(f => f.LastWriteTime).ToArray();
        ...
        {
            PutInternal(str, LogType.Fatal, false);
        }
        ...
        {
            PutInternal(str, LogType.Warn, false);
        }
        ...
        private static void PutInternal(string str, LogType reason, bool cleanMultiline = true)
        {
            ...
            var starter = $"[{GetPutTime()}] ";
            var reasonStr = reason.logCaller ? $"{reason.name} @ {GetCaller()}" : reason.name;
            var fileString = $"{starter}{reasonStr} -> {str}";
        ...
        private static string GetPutTime()
        {
            return DateTime.Now.ToString(PUT_DATE_FORMAT).PadLeft(MaxPutDateLength);
        }
        ...
        private static string GetCaller()
        {
            ...
            foreach (var frame in trace.GetFrames())
            {
                ...
                if (decType != typeof(Logger) && IsTypeLoggable(decType))
                {
        ...
        private static bool IsTypeLoggable(Type t)
        {
        ...
        private static bool IsMoveNext(Type t)
        {
        ...
        private static bool IsStepThrough(MethodBase m)
        {
